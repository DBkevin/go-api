...
func NewGormLogger() GormLogger {
	return GormLogger{
		...
		SlowThreshold: 200 * time.Microsecond, // 慢查询阈值，单位为千分之一秒
	}
}
...
// Info 实现gormlogger.Interface 的info方法
...
func (l GormLogger) logger() *zap.Logger {
	// 跳过 gorm 内置的调用
	var (
		gormPackge     = filepath.Join("gorm.io", "gorm")
	...
	for i := 2; i < 15; i++ {
		...
		switch {
		case !ok:
		case strings.HasSuffix(file, "_tets.go"):
		case strings.Contains(file, gormPackge):
